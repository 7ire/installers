...
Updates the mirrorlist to use the fastest mirrors using **reflector**.
...
- **Format the root partition**: Depending on your configuration, format the root partition as Btrfs or EXT4.
  ...
  - **For Btrfs**:
    ...
    ``` bash
    mkfs.btrfs -L archlinux /dev/mapper/cryptdev  # (or /dev/sda2 if not encrypted)
    mount /dev/mapper/cryptdev /mnt               # (or /dev/sda2)
    ```
...
> Ensure you mount the Btrfs subvolumes in the correct directories.
  ...
  - **For EXT4**:
    ...
    ``` bash
    mkfs.ext4 -L archlinux /dev/mapper/cryptdev  # or /dev/sda2 if not encrypted)
    mount /dev/mapper/cryptdev /mnt              # (or /dev/sda2)
    ```
